...
\end{equation}
be our modified periodogram, its limiting version should be 
\begin{equation}
...
Let $\hat{\theta}_j^{(r)}$ be the matrix of size $p\times p$, each element equals  $\hat{\theta}^{(r)}_{j, rs}$ and we define $\hat{\theta}_j^{(i)}$ in a similar way.  For real part, we can define following adaptive thresholding operator:
\begin{equation}
...
\begin{algorithm2e}[t]\small
	...
	\For{$\lambda \in \mathcal{L}$}{
		\For{$\nu \gets 1$ \textbf{to} $N$}{
			...
			$g_{1, \nu} (\omega_j) \gets \frac{1}{|J_1|}\sum_{k \in J_1} {\bf Re}(H(\omega_k))$, ~~ $g_{2, \nu} (\omega_j) \gets \sum_{k \in J_2} H(\omega_k)$\;
			$\hat{R}_{\nu}(\omega_j, \lambda) \gets \frac{1}{|J_2|}\left\| T_{\hat{\theta}^{(r)}_j\lambda} (g_{1,\nu} (\omega_j)) - g_{2,\nu}(\omega_j)\right\|^2_F$\;
		}
